...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
	...
	return (
		<html lang={locale} suppressHydrationWarning>
			<body className="bg-white dark:bg-neutral-950 selection:bg-neutral-950 dark:selection:bg-neutral-50 selection:text-white dark:selection:text-neutral-950 text-neutral-950/75 dark:text-white/75 focus-visible:outline-hidden focus-visible:ring-2 overflow-x-hidden">
				<ThemeProvider defaultTheme="dark" themes={["light", "dark", "system"]} attribute="class">
					<noscript>
						<div className="selection:bg-red text-white fixed flex z-100 bottom-6 left-1/2 -translate-x-1/2 min-h-9 w-[92vw] max-w-max bg-linear-to-b from-red-800/75 to-red-900/75 ring-inset ring-1 ring-red/10 border border-neutral-950 rounded-[18px] font-sans backdrop-blur-md shadow-xl shadow-neutral-950/50">
							<div className="inline-flex items-center p-2.5 rounded-3xl bg-linear-to-b from-red/50 to-red/25">
								<WarningTriangle />
							</div>
							<p className="px-3 py-2 text-sm">
								<span className="font-bold animate-pulse">No JavaScript detected.</span> Some parts of
								the site won't work properly. Please allow JavaScript on the site or use a more recent
								browser.
							</p>
						</div>
					</noscript>
