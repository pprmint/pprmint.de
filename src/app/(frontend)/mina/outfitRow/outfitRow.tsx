...
import { Artwork, Outfit } from "@/payload-types";
	...
	function CyclingFrontBackRef({
		...
		return (
			<div className="size-full" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
				...
				<AnimatePresence mode="wait">
					...
					>
						<AnimatePresence mode="wait">
							...
							>
								<Media
									resource={showBack ? data[selected].referenceBack : data[selected].referenceFront}
									alt={
										(typeof data[selected].referenceBack === "object" && data[selected].referenceBack.alt) ||
										(typeof data[selected].referenceFront === "object" && data[selected].referenceFront.alt) ||
										""
									}
									size="fhd"
				...
				<div className="flex text-sm h-9 border-y md:border-b-0 border-black/5 dark:border-white/5">
					<div className="grid grid-cols-2 grow">
						<button
							onClick={() => setShowBack(false)}
							className="relative group cursor-pointer flex items-center justify-center hover:bg-black/5 dark:hover:bg-white/5 border-r border-black/5 dark:border-white/5 duration-100"
						>
							<div
								className={`${
									!showBack
					...
					<button
						className="w-9 cursor-pointer flex items-center justify-center hover:bg-black/5 dark:hover:bg-white/5"
			...
			<Dialog.Portal>
				...
				<Dialog.Content className="fixed z-100 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-svh md:h-auto md:max-h-[80vh] w-screen max-w-6xl max-h-screen bg-white dark:bg-neutral-950 md:outline-solid outline-1 outline-black/5 dark:outline-white/5 data-[state=open]:animate-dialog-enter data-[state=closed]:animate-dialog-exit origin-center shadow-2xl md:grid grid-cols-2 items-center overflow-auto">
					...
					<div className="p-6 md:p-9">
						...
						{typeof data.designer === "object" && data.designer.creditLinks && data.designer.creditLinks.length > 0 ? (
							<p className="text-lg text-neutral-950 dark:text-white">
								{t.rich("Content.Outfits.designedBy", {
									designer: (typeof data.designer === "object" && data.designer.name) || "",
									Link: (chunks) => (
										<Link
						...
						<Dialog.Description className="mb-9">{data.description}</Dialog.Description>
						<Dialog.Close asChild>
							<Button design="semi-transparent" onClick={() => handleSelectOutfit(data.slug)}>
