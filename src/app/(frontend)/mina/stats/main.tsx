				...
				<Dialog.Content className="fixed z-100 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 max-h-svh sm:max-h-4/5-screen overflow-y-auto overflow-x-hidden md:h-max w-screen sm:w-[93vw] max-w-6xl bg-white dark:bg-neutral-950 outline-solid outline-1 outline-black/5 dark:outline-white/5 data-[state=open]:animate-dialog-enter data-[state=closed]:animate-dialog-exit origin-center shadow-2xl">
					...
					<div className="grid grid-rows-3 sm:grid-rows-2 xl:grid-rows-1 sm:grid-cols-2 xl:grid-cols-3 w-full gap-y-6 my-6 md:my-12">
						<div className="text-center">
							...
							<p className="text-lg">
								{t("Content.Artworks.Statistics.total")}
							</p>
						</div>
						<div className="text-center">
							<div className="font-mono text-6xl md:text-7xl font-thin font-stretch-expanded text-neutral-950 dark:text-white">
							...
							<p className="text-lg">
								{t("Content.Artworks.Statistics.commissioned")}
							</p>
						</div>
						<div className="text-center sm:col-span-2 xl:col-span-1">
							...
							<p className="text-lg">
								{t("Content.Artworks.Statistics.totalSpent")}
								<Image
									src="/api/artwork/file/layer_14155f4b0d.webp"
									alt="ehehe"
									width={622}
									height={583}
									className="inline h-6 ml-2 w-auto"
								/>
							</p>
						</div>
					</div>
					...
					<table className="w-full max-w-96 mx-auto">
						...
						<tbody>
							{artists.docs
								.sort(
									(a, b) =>
										(b.artworks?.totalDocs || 0) - (a.artworks?.totalDocs || 0),
								)
								.map((artist, _) => (
									<tr
										key={artist.id}
										className="odd:bg-black/2.5 dark:odd:bg-white/2.5"
									>
										<td className="px-2 py-1 text-left">{artist.name}</td>
										<td className="px-2 py-1 text-right">
											{artist.artworks?.totalDocs}
										</td>
									</tr>
								))}
						</tbody>
