...
export default function Gallery({ artworks, page }: { artworks: PaginatedDocs<Mina>; page: number }) {
	...
	function ContextMenuItem(props: React.PropsWithChildren<{ action?: MouseEventHandler<HTMLDivElement> }>) {
		return (
			<ContextMenu.Item className="group relative flex items-center gap-3 pr-2 pl-2 h-7 leading-none select-none outline-none focus:outline-none data-disabled:text-black/25 dark:data-disabled:text-white/25 data-disabled:pointer-events-none data-highlighted:text-neutral-950 dark:data-highlighted:text-white data-[state=checked]:text-neutral-950 dark:data-[state=checked]:text-white data-highlighted:bg-black/10 dark:data-highlighted:bg-white/10 active:opacity-75 duration-75 data-highlighted:duration-0 cursor-pointer focus-visible:outline-hidden" onClick={props.action}>
				{props.children}
			</ContextMenu.Item>
		)
	}
	// Show artwork by artist.
																	...
																	{typeof artworks.docs[selectedArtwork].images[
																		selectedVariant
																			...
																			<FadingImage
																				...
																				className={`max-h-svh w-auto mx-auto py-16 ${artworks.docs[selectedArtwork]
																		...
																		<div className="flex flex-row items-center justify-center h-9">
																			{artworks.docs[selectedArtwork].images.map(
																				(_, index) => (
																					<button
																						key={index}
																						className={`group h-full ${index === selectedVariant
																							? "w-12"
																							: "w-6"
																					...
																					>
																						<div
																							className={`${index === selectedVariant
															...
															>
																{artworks.docs.map((artwork, index) => (
																	<button
																		...
																		className={`relative ${selectedArtwork === index
																			? "h-12 w-16"
																	...
																	>
																		{typeof artwork.images[0].image === "object" && (
																			<Image
																				...
																				className={`absolute top-0 inset-x-0 h-full object-cover ${artwork.nsfw &&
											...
											<ContextMenu.Group>
												...
												{typeof artwork.images[0].image === "object" && artwork.images[0].image.url && (
													<Link href={artwork.images[0].image.url} target="_blank" rel="noopener noreferrer">
														<ContextMenuItem>{t("Content.Artworks.ContextMenu.Artwork.openInNewTab")}</ContextMenuItem>
													</Link>
												)}
												<ContextMenuItem action={() => navigator.clipboard.writeText(`${process.env.NEXT_PUBLIC_SERVER_URL}${typeof artwork.images[0].image === "object" && artwork.images[0].image.url}`)}>{t("Content.Artworks.ContextMenu.Artwork.copyImageUrl")}</ContextMenuItem>
												<ContextMenuItem action={() => saveAs(typeof artwork.images[0].image === "object" && artwork.images[0].image.url || "", typeof artwork.images[0].image === "object" && artwork.images[0].image.filename || "")}>{t("Content.Artworks.ContextMenu.Artwork.saveImage")}</ContextMenuItem>
											</ContextMenu.Group>
											<ContextMenu.Group>
												<ContextMenu.Label className="text-xs text-neutral-950/50 dark:text-white/50 ml-2 mt-1.5">{t("Content.Artworks.ContextMenu.Artist.label")}</ContextMenu.Label>
												<ContextMenuItem action={() => selectArtist(typeof artwork.artist === "object" && artwork.artist.slug || "")}>{t("Content.Artworks.ContextMenu.Artist.showAll", { artist: typeof artwork.artist === "object" && artwork.artist.name || "" })}</ContextMenuItem>
												{typeof artwork.artist === "object" && artwork.artist.creditUrl && (
													<Link href={artwork.artist.creditUrl} target="_blank" rel="noopener noreferrer">
														<ContextMenuItem>{t("Content.Artworks.ContextMenu.Artist.visitProfile", { site: new URL(artwork.artist.creditUrl).hostname })}</ContextMenuItem>
													</Link>
												)}
											</ContextMenu.Group>
										</ContextMenu.Content>
