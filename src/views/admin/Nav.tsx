...
function Nav() {
	...
	function NavLink(props: NavItem) {
		return (
			<Link
				href={props.link}
				className={`group flex gap-4 items-center ${!props.parent ? `border-0 ${props.link === "/admin" ? "border-y" : "border-t"} border-solid border-(--theme-elevation-100) h-12 px-4` : "h-9 pl-[59px] pr-4 text-sm"} text-(--theme-text) no-underline ${isActive(props.link) ? "bg-(--theme-elevation-100) font-bold" : "hover:bg-(--theme-elevation-50)"}`}
			>
				{!props.parent && (
					<div
						className={`size-[27px] rounded-full ${isActive(props.link) ? "bg-(--theme-text) text-(--theme-bg)" : "text-(--theme-text) bg-(--theme-elevation-50) group-hover:bg-(--theme-elevation-100)"}`}
					>
	...
	function NavSection({ title, items }: { title: string; items: NavItem[] }) {
		return (
			<div className="border-0 border-b border-solid border-(--theme-elevation-100)">
				<div className="text-(--theme-elevation-400) px-4 mb-2">
			...
			<div className="relative h-14">
				...
				>
					<svg
						...
						viewBox="0 0 972 800"
						className="absolute top-1 group-hover:top-0.5 group-active:top-1 w-full h-auto fill-(--theme-elevation-100) group-hover:fill-(--theme-elevation-200) duration-300 active:duration-75 ease-in-out-custom"
					>
