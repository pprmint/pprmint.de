...
export default function Checkbox({ color = "neutral", ...rest }: CheckboxProps) {
	...
	return (
		<RCheckbox.Root
			className={`group ${colors} flex items-center justify-center size-6 rounded-md outline-none focus:outline hover:brightness-110 active:brightness-90 active:shadow-inner duration-100 active:duration-50 border ${
				rest.disabled
		...
		>
			<RCheckbox.Indicator>
				<Check strokeWidth={3} size={16} className="drop-shadow-[0px_1px_1px_#0006] group-data-[state=checked]:animate-lucide-check-draw-in" strokeDasharray={24} />
			</RCheckbox.Indicator>
		</RCheckbox.Root>
	);
}
