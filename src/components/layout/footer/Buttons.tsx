...
export default function Buttons({ buttons }: { buttons: PaginatedDocs<ButtonType> }) {
		...
		<div className="flex justify-center">
			...
			{buttons.docs.length > 0 &&
				buttons.docs.map((button) => (
					<Link key={button.id} href={button.link} target="_blank" rel="noopener noreferrer">
						<Image
							unoptimized
							src={(button.url && typeof button.url === "string" && button.url) || ""}
							width={88}
							height={31}
							alt={button.alt}
						/>
